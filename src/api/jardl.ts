...
const JarDLBuildInfoSchema = z.object({
  ...
  isJdkRequired: z.boolean().nullable(),
  updatedDatetime: z.string().nullable(),
  recommended: z.boolean(),
  isBuildRequired: z.boolean(),
  isInfoLoaded: z.boolean(),
}).transform((data) => ({
  build: data.build,
  downloadUrl: data.downloadUrl,
  javaMajorVersion: data.javaMajorVersion,
  isJdkRequired: data.isJdkRequired,
  updatedAt: data.updatedDatetime ? dayjs.utc(data.updatedDatetime) : null,
  recommended: data.recommended,
  isBuildRequired: data.isBuildRequired,
  isInfoLoaded: data.isInfoLoaded,
}));
